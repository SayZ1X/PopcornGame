...
AsPlatform::AsPlatform()
 : X_Pos(AsConfig::Border_X_Offset), X_Step(AsConfig::Global_Scale * 2), Platform_State(EPS_Normal), Inner_Width(Normal_Platform_Inner_Width),
   Rolling_Step(0), Normal_Platform_Image_Width(0), Normal_Platform_Image_Height(0), Normal_Platform_Image(0), Meltdown_Platform_Y_Pos{},
   ...
   Platform_Circle_Pen_Color(209, 51, 88), Platform_Inner_Pen_Color(69, 238, 255)
{
...
{
   RECT intersection_rect, falling_letter_cell;
   
   falling_letter->Get_Letter_Cell(falling_letter_cell);

   if ( IntersectRect(&intersection_rect, &falling_letter_cell, &Platform_Rect) )
      return true;
   else
      return false;
}
