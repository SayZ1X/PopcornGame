...
{// Инициализация компонентов для создания платформы
   ...
   AsConfig::Create_Pen_Brush(209, 51, 88, Platform_Circle_Pen, Platform_Circle_Brush);
   AsConfig::Create_Pen_Brush(69, 238, 255, Platform_Inner_Pen, Platform_Inner_Brush);
};
//-------------------------------------------------------------------------------------------------------------------------
void AsPlatform::Act()
{// Вызываем перерисовку платформы для разных состояний
...
{// Геттер для получения состояния платформы
   return Platform_State;
}
//-------------------------------------------------------------------------------------------------------------------------
void AsPlatform::Set_State(EPlatform_State new_state)
{// Установка состояния платформы
   int i, len;
...
{// Рисуем платформу в нормальном состоянии
   ...
   // 1. Затираем фоном прошлую позицию
   Clear_BG(hdc);
   ...
   RoundRect(hdc, (x + 4) * AsConfig::Global_Scale, (y + 1) * AsConfig::Global_Scale, (x + 4 + Inner_Width - 1) * AsConfig::Global_Scale - 1, (y + 1 + 5) * AsConfig::Global_Scale - 1,
      3 * AsConfig::Global_Scale, 3 * AsConfig::Global_Scale);
}
//-------------------------------------------------------------------------------------------------------------------------
void AsPlatform::Draw_Meltdown_State(HDC hdc, RECT& paint_area)
{// Рисуем платформу в состоянии расплавления
   int i, j;
   ...
   max_platform_y = AsConfig::Max_Y_Pos * AsConfig::Global_Scale + area_height + 1;
...
{// Рисуем выкатывающуюся платформу
   ...
   double alpha = -(2.0 * M_PI) / (double)Max_Rolling_Step * Rolling_Step;
...
{// Рисуем расширение платформы после выкатывания 
   Draw_Normal_State(hdc, paint_area);

   --X_Pos;
   Inner_Width += 2;

   if (Inner_Width >= Normal_Platform_Inner_Widht)
   {
      Inner_Width = Normal_Platform_Inner_Widht;
